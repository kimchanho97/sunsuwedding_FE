...
👉 [https://sunsu-wedding.shop/](https://sunsu-wedding.shop/)

> 현재 배포 중인 순수웨딩 2.0 서비스입니다.
...
### 📌 트랜잭션 기반 결제 처리 (Toss Payments 연동)

**결제 무결성을 보장하고, 유저 등급 변경까지 원자적으로 처리하기 위해 트랜잭션 기반 결제 로직을 설계했습니다.**

- 토스 페이먼츠 연동으로 다양한 결제 수단 지원
- 결제 인증 → 서버 검증 → 결제 승인 + 유저 등급 업그레이드까지 **하나의 트랜잭션으로 처리**
- 외부 API 오류 또는 실패 시, 데이터 롤백 처리로 무결성 유지
